...
The [`blueberry-jetson-containers build`](/blueberry-jetson-containers) command is a proxy launcher for [`blueberry_jetson_containers/build.py`](/blueberry_jetson_containers/build.py).  It can be run from any working directory after you clone the repo and run the installer from the [System Setup](/docs/setup.md) (you should also probably mount additional storage when building containers)

To list the packages available to build for your version of JetPack/L4T, you can use `--list-packages` and `--show-packages`:

```bash
$ blueberry-jetson-containers build --list-packages       # list all packages
$ blueberry-jetson-containers build --show-packages       # show all package metadata
$ blueberry-jetson-containers build --show-packages ros*  # show all the ros packages
```

To build a container that includes one or more packages:

```bash
$ blueberry-jetson-containers build pytorch               # build a container with just PyTorch
$ blueberry-jetson-containers build pytorch jupyterlab    # build container with PyTorch and JupyterLab
```
...
The docker commands that get run during the build are printed and also saved to shell scripts under the `blueberry-jetson-containers/logs` directory.  To see just the commands it would have run without actually running them, use the `--simulate` flag.
...
```bash
blueberry-jetson-containers build --name=my_container pytorch jupyterlab
```
...
```bash
$ blueberry-jetson-containers build --multiple pytorch tensorflow2   # built a pytorch container and a tensorflow2 container
$ blueberry-jetson-containers build --multiple ros*                  # build all ROS containers
```
...
```bash
blueberry-jetson-containers build --base=my_container:latest --name=my_container:pytorch pytorch  # add pytorch to your container
```
...
```bash
CUDA_VERSION=12.4 blueberry-jetson-containers build --name=cu124/ pytorch  # build PyTorch for CUDA 12.4
```
...
CUDA_VERSION=12.4 PYTHON_VERSION=3.11 PYTORCH_VERSION=2.3 \
  blueberry-jetson-containers build l4t-text-generation
```
...
```bash
blueberry-jetson-containers build --package-dirs=/path/to/your/package your_package_name
```

You can also add blueberry-jetson-containers as a git submodule to your project and build it that way (see [jetson-inference](https://github.com/dusty-nv/jetson-inference) as an example of this)
...
``` bash
$ blueberry-jetson-containers build --skip-tests=numpy,onnx pytorch    # skip testing the numpy and onnx packages when building pytorch
$ blueberry-jetson-containers build --skip-tests=all pytorch           # skip all tests
$ blueberry-jetson-containers build --skip-tests=intermediate pytorch  # only run tests at the end of the container build
```
...
```
$ blueberry-jetson-containers build --no-github-api --skip-test=all text-generation-webui
```
