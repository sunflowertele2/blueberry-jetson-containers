...
blueberry-jetson-containers run $(autotag protobuf_apt)
...
```bash
blueberry-jetson-containers run -v /path/on/host:/path/in/container $(autotag protobuf_apt)
```
To launch the container running a command, as opposed to an interactive shell:
```bash
blueberry-jetson-containers run $(autotag protobuf_apt) my_app --abc xyz
```
...
```bash
blueberry-jetson-containers build protobuf_apt
```
