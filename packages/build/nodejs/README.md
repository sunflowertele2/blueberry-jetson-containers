...
blueberry-jetson-containers run $(autotag nodejs)
...
```bash
blueberry-jetson-containers run -v /path/on/host:/path/in/container $(autotag nodejs)
```
To launch the container running a command, as opposed to an interactive shell:
```bash
blueberry-jetson-containers run $(autotag nodejs) my_app --abc xyz
```
...
```bash
blueberry-jetson-containers build nodejs
```
