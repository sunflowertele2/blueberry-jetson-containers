...
blueberry-jetson-containers run $(autotag python)

# or explicitly specify one of the container images above
blueberry-jetson-containers run dustynv/python:r32.7.1
...
```bash
blueberry-jetson-containers run -v /path/on/host:/path/in/container $(autotag python)
```
To launch the container running a command, as opposed to an interactive shell:
```bash
blueberry-jetson-containers run $(autotag python) my_app --abc xyz
```
...
```bash
blueberry-jetson-containers build python
```
