...
blueberry-jetson-containers run $(autotag piper)
...
```bash
blueberry-jetson-containers run -v /path/on/host:/path/in/container $(autotag piper)
```
To launch the container running a command, as opposed to an interactive shell:
```bash
blueberry-jetson-containers run $(autotag piper) my_app --abc xyz
```
...
```bash
blueberry-jetson-containers build piper
```
