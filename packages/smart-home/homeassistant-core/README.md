...
blueberry-jetson-containers run $(autotag homeassistant-core)

# or explicitly specify one of the container images above
blueberry-jetson-containers run dustynv/homeassistant-core:latest-r36.2.0
...
```bash
blueberry-jetson-containers run -v /path/on/host:/path/in/container $(autotag homeassistant-core)
```
To launch the container running a command, as opposed to an interactive shell:
```bash
blueberry-jetson-containers run $(autotag homeassistant-core) my_app --abc xyz
```
...
```bash
blueberry-jetson-containers build homeassistant-core
```
