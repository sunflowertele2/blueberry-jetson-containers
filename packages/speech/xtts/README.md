...
blueberry-jetson-containers run $(autotag xtts)

# or explicitly specify one of the container images above
blueberry-jetson-containers run dustynv/xtts:r36.3.0
...
```bash
blueberry-jetson-containers run -v /path/on/host:/path/in/container $(autotag xtts)
```
To launch the container running a command, as opposed to an interactive shell:
```bash
blueberry-jetson-containers run $(autotag xtts) my_app --abc xyz
```
...
```bash
blueberry-jetson-containers build xtts
```
