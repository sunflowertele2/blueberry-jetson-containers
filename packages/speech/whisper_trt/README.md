...
blueberry-jetson-containers run $(autotag whisper_trt)

# or explicitly specify one of the container images above
blueberry-jetson-containers run dustynv/whisper_trt:r36.3.0
...
```bash
blueberry-jetson-containers run -v /path/on/host:/path/in/container $(autotag whisper_trt)
```
To launch the container running a command, as opposed to an interactive shell:
```bash
blueberry-jetson-containers run $(autotag whisper_trt) my_app --abc xyz
```
...
```bash
blueberry-jetson-containers build whisper_trt
```
