...
blueberry-jetson-containers run $(autotag faster-whisper)
...
```bash
blueberry-jetson-containers run -v /path/on/host:/path/in/container $(autotag faster-whisper)
```
To launch the container running a command, as opposed to an interactive shell:
```bash
blueberry-jetson-containers run $(autotag faster-whisper) my_app --abc xyz
```
...
```bash
blueberry-jetson-containers build faster-whisper
```
