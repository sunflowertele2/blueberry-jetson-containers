...
blueberry-jetson-containers run $(autotag comfyui)

# or explicitly specify one of the container images above
blueberry-jetson-containers run dustynv/comfyui:r36.3.0
...
```bash
blueberry-jetson-containers run -v /path/on/host:/path/in/container $(autotag comfyui)
```
To launch the container running a command, as opposed to an interactive shell:
```bash
blueberry-jetson-containers run $(autotag comfyui) my_app --abc xyz
```
...
```bash
blueberry-jetson-containers build comfyui
```
