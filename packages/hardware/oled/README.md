...
blueberry-jetson-containers run $(autotag oled)
...
```bash
blueberry-jetson-containers run -v /path/on/host:/path/in/container $(autotag oled)
```
To launch the container running a command, as opposed to an interactive shell:
```bash
blueberry-jetson-containers run $(autotag oled) my_app --abc xyz
```
...
```bash
blueberry-jetson-containers build oled
```
