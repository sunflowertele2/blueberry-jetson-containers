...
blueberry-jetson-containers run $(autotag jetcam)
...
```bash
blueberry-jetson-containers run -v /path/on/host:/path/in/container $(autotag jetcam)
```
To launch the container running a command, as opposed to an interactive shell:
```bash
blueberry-jetson-containers run $(autotag jetcam) my_app --abc xyz
```
...
```bash
blueberry-jetson-containers build jetcam
```
