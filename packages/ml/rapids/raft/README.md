...
blueberry-jetson-containers run $(autotag raft)
...
```bash
blueberry-jetson-containers run -v /path/on/host:/path/in/container $(autotag raft)
```
To launch the container running a command, as opposed to an interactive shell:
```bash
blueberry-jetson-containers run $(autotag raft) my_app --abc xyz
```
...
```bash
blueberry-jetson-containers build raft
```
