...
blueberry-jetson-containers run $(autotag mamba)

# or explicitly specify one of the container images above
blueberry-jetson-containers run dustynv/mamba:r36.3.0
...
```bash
blueberry-jetson-containers run -v /path/on/host:/path/in/container $(autotag mamba)
```
To launch the container running a command, as opposed to an interactive shell:
```bash
blueberry-jetson-containers run $(autotag mamba) my_app --abc xyz
```
...
```bash
blueberry-jetson-containers build mamba
```
