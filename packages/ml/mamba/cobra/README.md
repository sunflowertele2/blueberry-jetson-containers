...
blueberry-jetson-containers run $(autotag cobra)
...
```bash
blueberry-jetson-containers run -v /path/on/host:/path/in/container $(autotag cobra)
```
To launch the container running a command, as opposed to an interactive shell:
```bash
blueberry-jetson-containers run $(autotag cobra) my_app --abc xyz
```
...
```bash
blueberry-jetson-containers build cobra
```
