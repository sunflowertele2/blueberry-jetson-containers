...
blueberry-jetson-containers run $(autotag dimba)
...
```bash
blueberry-jetson-containers run -v /path/on/host:/path/in/container $(autotag dimba)
```
To launch the container running a command, as opposed to an interactive shell:
```bash
blueberry-jetson-containers run $(autotag dimba) my_app --abc xyz
```
...
```bash
blueberry-jetson-containers build dimba
```
