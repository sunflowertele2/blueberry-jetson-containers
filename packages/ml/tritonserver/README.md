...
blueberry-jetson-containers run $(autotag tritonserver)

# or explicitly specify one of the container images above
blueberry-jetson-containers run dustynv/tritonserver:r35.4.1
...
```bash
blueberry-jetson-containers run -v /path/on/host:/path/in/container $(autotag tritonserver)
```
To launch the container running a command, as opposed to an interactive shell:
```bash
blueberry-jetson-containers run $(autotag tritonserver) my_app --abc xyz
```
...
```bash
blueberry-jetson-containers build tritonserver
```
