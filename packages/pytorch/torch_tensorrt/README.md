...
blueberry-jetson-containers run $(autotag torch_tensorrt)

# or explicitly specify one of the container images above
blueberry-jetson-containers run dustynv/torch_tensorrt:r35.3.1
...
```bash
blueberry-jetson-containers run -v /path/on/host:/path/in/container $(autotag torch_tensorrt)
```
To launch the container running a command, as opposed to an interactive shell:
```bash
blueberry-jetson-containers run $(autotag torch_tensorrt) my_app --abc xyz
```
...
```bash
blueberry-jetson-containers build torch_tensorrt
```
