...
blueberry-jetson-containers run $(autotag openexr)
...
```bash
blueberry-jetson-containers run -v /path/on/host:/path/in/container $(autotag openexr)
```
To launch the container running a command, as opposed to an interactive shell:
```bash
blueberry-jetson-containers run $(autotag openexr) my_app --abc xyz
```
...
```bash
blueberry-jetson-containers build openexr
```
