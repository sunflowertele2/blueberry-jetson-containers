...
blueberry-jetson-containers run $(autotag deepstream)

# or explicitly specify one of the container images above
blueberry-jetson-containers run dustynv/deepstream:r32.7.1
...
```bash
blueberry-jetson-containers run -v /path/on/host:/path/in/container $(autotag deepstream)
```
To launch the container running a command, as opposed to an interactive shell:
```bash
blueberry-jetson-containers run $(autotag deepstream) my_app --abc xyz
```
...
```bash
blueberry-jetson-containers build deepstream
```
