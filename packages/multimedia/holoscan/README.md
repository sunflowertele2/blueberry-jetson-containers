...
blueberry-jetson-containers run $(autotag holoscan)

# or explicitly specify one of the container images above
blueberry-jetson-containers run dustynv/holoscan:r36.3.0
...
```bash
blueberry-jetson-containers run -v /path/on/host:/path/in/container $(autotag holoscan)
```
To launch the container running a command, as opposed to an interactive shell:
```bash
blueberry-jetson-containers run $(autotag holoscan) my_app --abc xyz
```
...
```bash
blueberry-jetson-containers build holoscan
```
