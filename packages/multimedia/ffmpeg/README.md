...
blueberry-jetson-containers run $(autotag ffmpeg)
...
```bash
blueberry-jetson-containers run -v /path/on/host:/path/in/container $(autotag ffmpeg)
```
To launch the container running a command, as opposed to an interactive shell:
```bash
blueberry-jetson-containers run $(autotag ffmpeg) my_app --abc xyz
```
...
```bash
blueberry-jetson-containers build ffmpeg
```
