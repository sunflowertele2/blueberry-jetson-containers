...
blueberry-jetson-containers run $(autotag vila)

# or explicitly specify one of the container images above
blueberry-jetson-containers run dustynv/vila:r36.3.0
...
```bash
blueberry-jetson-containers run -v /path/on/host:/path/in/container $(autotag vila)
```
To launch the container running a command, as opposed to an interactive shell:
```bash
blueberry-jetson-containers run $(autotag vila) my_app --abc xyz
```
...
```bash
blueberry-jetson-containers build vila
```
