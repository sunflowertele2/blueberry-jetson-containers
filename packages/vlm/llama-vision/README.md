...
blueberry-jetson-containers run $(autotag llama-vision)

# or explicitly specify one of the container images above
blueberry-jetson-containers run dustynv/llama-vision:r36.3.0
...
```bash
blueberry-jetson-containers run -v /path/on/host:/path/in/container $(autotag llama-vision)
```
To launch the container running a command, as opposed to an interactive shell:
```bash
blueberry-jetson-containers run $(autotag llama-vision) my_app --abc xyz
```
...
```bash
blueberry-jetson-containers build llama-vision
```
