...
blueberry-jetson-containers run $(autotag kat)
...
```bash
blueberry-jetson-containers run -v /path/on/host:/path/in/container $(autotag kat)
```
To launch the container running a command, as opposed to an interactive shell:
```bash
blueberry-jetson-containers run $(autotag kat) my_app --abc xyz
```
...
```bash
blueberry-jetson-containers build kat
```
