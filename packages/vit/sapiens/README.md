...
blueberry-jetson-containers run $(autotag sapiens)
...
```bash
blueberry-jetson-containers run -v /path/on/host:/path/in/container $(autotag sapiens)
```
To launch the container running a command, as opposed to an interactive shell:
```bash
blueberry-jetson-containers run $(autotag sapiens) my_app --abc xyz
```
...
```bash
blueberry-jetson-containers build sapiens
```
