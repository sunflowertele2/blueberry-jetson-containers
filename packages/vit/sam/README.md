...
blueberry-jetson-containers run $(autotag sam)

# or explicitly specify one of the container images above
blueberry-jetson-containers run dustynv/sam:r36.2.0
...
```bash
blueberry-jetson-containers run -v /path/on/host:/path/in/container $(autotag sam)
```
To launch the container running a command, as opposed to an interactive shell:
```bash
blueberry-jetson-containers run $(autotag sam) my_app --abc xyz
```
...
```bash
blueberry-jetson-containers build sam
```
