...
blueberry-jetson-containers run $(autotag llama_cpp)

# or explicitly specify one of the container images above
blueberry-jetson-containers run dustynv/llama_cpp:r36.4.0
...
```bash
blueberry-jetson-containers run -v /path/on/host:/path/in/container $(autotag llama_cpp)
```
To launch the container running a command, as opposed to an interactive shell:
```bash
blueberry-jetson-containers run $(autotag llama_cpp) my_app --abc xyz
```
...
```bash
blueberry-jetson-containers build llama_cpp
```
