...
blueberry-jetson-containers run $(autotag auto_gptq)

# or explicitly specify one of the container images above
blueberry-jetson-containers run dustynv/auto_gptq:r36.2.0
...
```bash
blueberry-jetson-containers run -v /path/on/host:/path/in/container $(autotag auto_gptq)
```
To launch the container running a command, as opposed to an interactive shell:
```bash
blueberry-jetson-containers run $(autotag auto_gptq) my_app --abc xyz
```
...
```bash
blueberry-jetson-containers build auto_gptq
```
