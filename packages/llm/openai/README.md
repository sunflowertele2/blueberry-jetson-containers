...
blueberry-jetson-containers run $(autotag openai)
...
```bash
blueberry-jetson-containers run -v /path/on/host:/path/in/container $(autotag openai)
```
To launch the container running a command, as opposed to an interactive shell:
```bash
blueberry-jetson-containers run $(autotag openai) my_app --abc xyz
```
...
```bash
blueberry-jetson-containers build openai
```
