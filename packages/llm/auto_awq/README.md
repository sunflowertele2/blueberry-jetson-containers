...
blueberry-jetson-containers run $(autotag auto_awq)
...
```bash
blueberry-jetson-containers run -v /path/on/host:/path/in/container $(autotag auto_awq)
```
To launch the container running a command, as opposed to an interactive shell:
```bash
blueberry-jetson-containers run $(autotag auto_awq) my_app --abc xyz
```
...
```bash
blueberry-jetson-containers build auto_awq
```
