...
blueberry-jetson-containers run $(autotag flash-attention)
...
```bash
blueberry-jetson-containers run -v /path/on/host:/path/in/container $(autotag flash-attention)
```
To launch the container running a command, as opposed to an interactive shell:
```bash
blueberry-jetson-containers run $(autotag flash-attention) my_app --abc xyz
```
...
```bash
blueberry-jetson-containers build flash-attention
```
