...
blueberry-jetson-containers run $(autotag exllama)

# or explicitly specify one of the container images above
blueberry-jetson-containers run dustynv/exllama:v1-r36.2.0
...
```bash
blueberry-jetson-containers run -v /path/on/host:/path/in/container $(autotag exllama)
```
To launch the container running a command, as opposed to an interactive shell:
```bash
blueberry-jetson-containers run $(autotag exllama) my_app --abc xyz
```
...
```bash
blueberry-jetson-containers build exllama
```
