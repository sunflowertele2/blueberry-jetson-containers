...
blueberry-jetson-containers run $(autotag awq)

# or explicitly specify one of the container images above
blueberry-jetson-containers run dustynv/awq:r35.3.1
...
```bash
blueberry-jetson-containers run -v /path/on/host:/path/in/container $(autotag awq)
```
To launch the container running a command, as opposed to an interactive shell:
```bash
blueberry-jetson-containers run $(autotag awq) my_app --abc xyz
```
...
```bash
blueberry-jetson-containers build awq
```
