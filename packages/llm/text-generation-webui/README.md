...
blueberry-jetson-containers run $(autotag text-generation-webui)

# or explicitly specify one of the container images above
blueberry-jetson-containers run dustynv/text-generation-webui:r35.4.1-cp310
...
```bash
blueberry-jetson-containers run -v /path/on/host:/path/in/container $(autotag text-generation-webui)
```
To launch the container running a command, as opposed to an interactive shell:
```bash
blueberry-jetson-containers run $(autotag text-generation-webui) my_app --abc xyz
```
...
```bash
blueberry-jetson-containers build text-generation-webui
```
