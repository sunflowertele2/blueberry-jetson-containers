...
blueberry-jetson-containers run $(autotag gptq-for-llama)

# or explicitly specify one of the container images above
blueberry-jetson-containers run dustynv/gptq-for-llama:r36.2.0
...
```bash
blueberry-jetson-containers run -v /path/on/host:/path/in/container $(autotag gptq-for-llama)
```
To launch the container running a command, as opposed to an interactive shell:
```bash
blueberry-jetson-containers run $(autotag gptq-for-llama) my_app --abc xyz
```
...
```bash
blueberry-jetson-containers build gptq-for-llama
```
