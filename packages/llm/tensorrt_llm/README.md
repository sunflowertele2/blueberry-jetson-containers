...
blueberry-jetson-containers run $(autotag tensorrt_llm)

# or explicitly specify one of the container images above
blueberry-jetson-containers run dustynv/tensorrt_llm:0.12-r36.4.0
...
```bash
blueberry-jetson-containers run -v /path/on/host:/path/in/container $(autotag tensorrt_llm)
```
To launch the container running a command, as opposed to an interactive shell:
```bash
blueberry-jetson-containers run $(autotag tensorrt_llm) my_app --abc xyz
```
...
```bash
blueberry-jetson-containers build tensorrt_llm
```
