...
blueberry-jetson-containers run $(autotag optimum)

# or explicitly specify one of the container images above
blueberry-jetson-containers run dustynv/optimum:r35.2.1
...
```bash
blueberry-jetson-containers run -v /path/on/host:/path/in/container $(autotag optimum)
```
To launch the container running a command, as opposed to an interactive shell:
```bash
blueberry-jetson-containers run $(autotag optimum) my_app --abc xyz
```
...
```bash
blueberry-jetson-containers build optimum
```
