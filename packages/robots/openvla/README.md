...
blueberry-jetson-containers run $(autotag openvla)

# or explicitly specify one of the container images above
blueberry-jetson-containers run dustynv/openvla:r36.3.0
...
```bash
blueberry-jetson-containers run -v /path/on/host:/path/in/container $(autotag openvla)
```
To launch the container running a command, as opposed to an interactive shell:
```bash
blueberry-jetson-containers run $(autotag openvla) my_app --abc xyz
```
...
```bash
blueberry-jetson-containers build openvla
```
