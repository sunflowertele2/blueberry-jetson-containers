...
blueberry-jetson-containers run $(autotag cosmos)

# or explicitly specify one of the container images above
blueberry-jetson-containers run dustynv/cosmos:r36.4.0
...
```bash
blueberry-jetson-containers run -v /path/on/host:/path/in/container $(autotag cosmos)
```
To launch the container running a command, as opposed to an interactive shell:
```bash
blueberry-jetson-containers run $(autotag cosmos) my_app --abc xyz
```
...
```bash
blueberry-jetson-containers build cosmos
```
