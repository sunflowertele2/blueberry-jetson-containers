...
blueberry-jetson-containers run $(autotag mujoco)
...
```bash
blueberry-jetson-containers run -v /path/on/host:/path/in/container $(autotag mujoco)
```
To launch the container running a command, as opposed to an interactive shell:
```bash
blueberry-jetson-containers run $(autotag mujoco) my_app --abc xyz
```
...
```bash
blueberry-jetson-containers build mujoco
```
