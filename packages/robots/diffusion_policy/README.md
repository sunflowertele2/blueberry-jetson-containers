...
blueberry-jetson-containers run $(autotag diffusion_policy)

# or explicitly specify one of the container images above
blueberry-jetson-containers run dustynv/diffusion_policy:r36.3.0
...
```bash
blueberry-jetson-containers run -v /path/on/host:/path/in/container $(autotag diffusion_policy)
```
To launch the container running a command, as opposed to an interactive shell:
```bash
blueberry-jetson-containers run $(autotag diffusion_policy) my_app --abc xyz
```
...
```bash
blueberry-jetson-containers build diffusion_policy
```
