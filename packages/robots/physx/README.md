...
blueberry-jetson-containers run $(autotag physx)
...
```bash
blueberry-jetson-containers run -v /path/on/host:/path/in/container $(autotag physx)
```
To launch the container running a command, as opposed to an interactive shell:
```bash
blueberry-jetson-containers run $(autotag physx) my_app --abc xyz
```
...
```bash
blueberry-jetson-containers build physx
```
