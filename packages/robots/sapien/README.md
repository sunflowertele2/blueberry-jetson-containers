...
blueberry-jetson-containers run $(autotag sapien)
...
```bash
blueberry-jetson-containers run -v /path/on/host:/path/in/container $(autotag sapien)
```
To launch the container running a command, as opposed to an interactive shell:
```bash
blueberry-jetson-containers run $(autotag sapien) my_app --abc xyz
```
...
```bash
blueberry-jetson-containers build sapien
```
