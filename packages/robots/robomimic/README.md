...
blueberry-jetson-containers run $(autotag robomimic)
...
```bash
blueberry-jetson-containers run -v /path/on/host:/path/in/container $(autotag robomimic)
```
To launch the container running a command, as opposed to an interactive shell:
```bash
blueberry-jetson-containers run $(autotag robomimic) my_app --abc xyz
```
...
```bash
blueberry-jetson-containers build robomimic
```
