...
blueberry-jetson-containers run $(autotag faiss)

# or explicitly specify one of the container images above
blueberry-jetson-containers run dustynv/faiss:be12427-builder-r35.4.1
...
```bash
blueberry-jetson-containers run -v /path/on/host:/path/in/container $(autotag faiss)
```
To launch the container running a command, as opposed to an interactive shell:
```bash
blueberry-jetson-containers run $(autotag faiss) my_app --abc xyz
```
...
```bash
blueberry-jetson-containers build faiss
```
